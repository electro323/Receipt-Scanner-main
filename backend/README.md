...
- BMTC pickup and destination are extracted by first focusing on the standalone `TO` closest to the center of the ticket/image. The nearest readable English place above that `TO` is pickup, and the nearest readable English place below that `TO` is destination, skipping Kannada/noisy lines.
