export async function processReceiptWithAI(rawText: string) {
  ...
  const prompt = [
    ...
    'For train ticket journey stations, use only the top layout section. pickup_point is the station below Boarding, Boarding From, or Boarding At. destination is the station below To or the station printed above Arrival in the same top header area. Preserve station names and codes exactly, such as HOWRAH JN (HWH). Do not use city notes like (Howrah / Kolkata).',
