export async function processReceiptWithAI(rawText: string) {
  ...
  const prompt = [
    ...
    'For bus tickets, detect pickup_point and destination from the layout around the standalone word TO. For pickup_point, scan upward from TO: if the first line above TO is Kannada/non-English/noisy, ignore it and keep scanning upward until the first readable English place name. For destination, scan downward from TO: if the first line below TO is Kannada/non-English/noisy, ignore it and keep scanning downward until the first readable English place name. If the first line below TO is already English, use it. Keep place names exactly as OCR text when readable.',
    'For BMTC/Bangalore bus tickets, the word depot is a strong BMTC signal. Use the bus travel ticket schema. totals.total and payment.amount must come only from the amount printed on the same line as CASH or UPI. The fare is the two-digit rupee amount, for example Rs.15.00 (CASH) => 15 and Rs.35.00 (UPI) => 35. Do not use ticket number, phone number, depot number, route number, or GST line as total.',
