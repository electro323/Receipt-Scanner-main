export async function processReceiptWithAI(rawText: string) {
  ...
  const prompt = [
    ...
    'VERY IMPORTANT BMTC layout rule: in BMTC tickets, first find the standalone word TO in the center of the ticket/image. The readable English word/place name above TO is pickup_point. The readable English word/place name below TO is destination. If the line immediately above or below TO is Kannada/non-English/noisy, skip it and use the nearest readable English place on that side. Never use depot-only labels, timestamps, watermarks, phone numbers, ticket numbers, or random OCR noise as pickup_point or destination.',
