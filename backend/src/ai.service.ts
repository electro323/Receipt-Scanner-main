export async function processReceiptWithAI(rawText: string) {
  ...
  const prompt = [
    ...
    'VERY IMPORTANT travel/BMTC positional OCR rules: find the standalone English word TO, not TO inside another word. The English place immediately above TO is pickup_point. The English place immediately below TO is destination. Ignore Kannada text for pickup/destination when English text exists. Ignore depot names, route numbers, ticket numbers, timestamps, bus numbers, fare amounts, and metadata. Only choose text that looks like a place name: alphabetic words, possibly multiple words. If either pickup_point or destination cannot be confidently determined, leave it empty instead of guessing. Do not use regex or assumptions tied to specific stop names, city names, or sample receipts.',
